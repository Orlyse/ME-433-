...
void init_camera(){
    ...
    */
    uint8_t com8 = OV7670_read_register(OV7670_COM8_AWB);
    com8 = com8 & 0x1D;
    OV7670_write_register(OV7670_COM8_AWB, com8);   // disable AWB
    OV7670_write_register(OV7670_REG_RED, 0x50);
    OV7670_write_register(OV7670_REG_BLUE, 0x50);
...
bool line(int pixel, int row){
    int r = row*IMAGESIZEX; // find the index of the start of the row in the pixel array
    
    int red = picture.r[r+pixel];
    int green = picture.g[r+pixel];
    int blue = picture.b[r+pixel];
...
int findLine(int row, int *s, int *e){
    ...
    for(i=0;i<IMAGESIZEX;i++){
        int red = picture.r[r+i];
        int green = picture.g[r+i];
        int blue = picture.b[r+i];
